...
import { searchVehicles } from "@/src/store/vehicleSlice"

export default function Home_Search_Engine() {
    const { t } = useTranslation()
    const router = useRouter()
    const dispatch = useDispatch<AppDispatch>()
    const { loading, error } = useSelector((state: RootState) => state.vehicles)
    ...
    useEffect(() => {
        ...
        setReturnInfo(
            t("home.search.returnInfo", {
                pickup: pickupDate.toLocaleDateString("es-CO"),
                returnDate: returnDate.toLocaleDateString("es-CO"),
                hour: returnHour,
            })
        )
    }, [date, hours, t])
    ...
    const handleSearch = async () => {
        ...
        if (!location.trim()) {
            setValidationError(t("home.search.errorLocation"))
            return
        }
        if (!date) {
            setValidationError(t("home.search.errorDate"))
            return
        }
        if (!hours) {
            setValidationError(t("home.search.errorHours"))
            return
        }

        const result = await dispatch(searchVehicles(location))

        if (searchVehicles.fulfilled.match(result)) {
            router.push("/search")
        } else {
            setValidationError(t("home.search.errorSearch"))
        }
    }
        ...
        <section className="w-full h-auto flex flex-col justify-center items-center font-thin text-neutral-800">
            ...
            <section className="w-full h-auto bg-[#00e55e] flex flex-row items-center justify-between px-10 py-4">
                ...
                <button
                    ...
                    disabled={loading}
                    className="flex gap-4 items-center text-[10px] md:text-xs hover:text-indigo-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    <p>{loading ? "..." : t("home.search.searchButton")}</p>
                    <FiSearch />
                </button>
            </section>
            ...
            <section className="w-full flex flex-col bg-neutral-100 p-10 md:p-20 gap-4">
                ...
                <div className="flex flex-col w-full text-xs">
                    ...
                    <input
                        value={hours}
                        onChange={(e) => { setHours(e.target.value); setValidationError(null) }}
